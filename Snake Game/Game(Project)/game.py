...
# Sound Setup
sound_dir = "C:/Users/ZaroonTayyab/Documents/My Coding Projects/Snake Game/Game(Project)/sounds/"
music_muted = False
music_volume = 0.3

music_files = [f for f in os.listdir(sound_dir) if f.endswith('.mp3') and "bg" in f]
chosen_track = random.choice(music_files)
pygame.mixer.music.load(os.path.join(sound_dir, chosen_track))
pygame.mixer.music.set_volume(music_volume)
pygame.mixer.music.play(-1)

eat_sound = pygame.mixer.Sound(sound_dir + "eat.wav")
gameover_sound = pygame.mixer.Sound(sound_dir + "game_over.mp3")
...
def generate_food(obstacles, width, height):
    while True:
...
def gameLoop():
    global highest_score, pause, music_muted, music_volume
    ...
    while len(obstacles) < 5:
        ox, oy = random.randrange(0, width, 20), random.randrange(0, height, 20)
        if (ox, oy) not in obstacles:
            obstacles.add((ox, oy))

    food_x, food_y = generate_food(obstacles, width, height)
